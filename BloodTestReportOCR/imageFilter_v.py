...
class ImageFilter:
    ...
    def perspect(self, param=default):
        ...
        image_rect = cv2.polylines(image_rect, [pts],isClosed, color, thickness)
        ...
        self.PerspectiveImg = cv2.warpPerspective(self.img, PerspectiveMatrix, (1000, 525))
    ...
    def autocut(self, num, param=default):
        ...
        #输出年龄
        img_age = self.PerspectiveImg[0 : 25, 550 : 580]
        cv2.imwrite(self.output_path + 'age.jpg', img_age)

        #输出性别
        img_gender = self.PerspectiveImg[0 : 27, 300 : 380]
    ...
    def ocr(self, num):
        ...
        def image_to_string(image, flag=True):
            if flag:
                # text = pytesseract.image_to_string(Image.fromarray(image), config='--psm 7 -c tessedit_char_whitelist=01234567890.')
                text = pytesseract.image_to_string(Image.fromarray(image), config='--psm 7 digits')
            else:
                text = pytesseract.image_to_string(Image.fromarray(image), lang='chi_sim', config='--psm 7  -c tessedit_char_whitelist=男女')
            return text
      ...
      #      image = imgproc.digitsimg(image)
        ...
        data['profile']['gender'] = image_to_string(read('temp_pics/gender.jpg'),False).replace(" ", '').replace("\n", '').strip()
